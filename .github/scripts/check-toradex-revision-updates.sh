...
current_tag=$(grep -oP 'version:\s*"\K[^"]+' \
    $SCRIPT_DIR/../../template/torizon-dtb-overlay-nxp.yaml.tpl) || {
...
if [ "$latest_stable_tag" != "$current_tag" ]; then
    ...
    git -C $SCRIPT_DIR/../../linux-toradex checkout $linux_toradex_nxp_commit || {
    ...
    sed -i "s/_Tezi_$current_tag/_Tezi_$latest_stable_tag/g" \
        $SCRIPT_DIR/../../template/torizon-dtb-overlay-nxp.yaml.tpl || {
        echo -e "Failed to update the torizon-dtb-overlay-nxp.yaml.tpl template file with the new Toradex tag.\n"
        exit 1
    }
